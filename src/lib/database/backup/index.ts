...
export class DatabaseBackupManager {
    ...
    try {
      ...
      for (const table of tables) {
        ...
        if (since) {
          ...
          query = query.or(
            `updated_at.gte.${since.toISOString()},created_at.gte.${since.toISOString()}`
          );
        }
  ...
  private async dropAllTables(): Promise<void> {
    ...
    for (const table of tables || []) {
      try {
        const { error: dropError } = await this.supabase.rpc('drop_table', {
  ...
  // Get backup history
  async getBackupHistory(directory: string): Promise<
    Array<{
      file: string;
      size: number;
      created: Date;
