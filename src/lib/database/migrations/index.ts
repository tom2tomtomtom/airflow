...
export class MigrationManager {
  ...
  async loadMigrations(): Promise<Migration[]> {
    try {
      ...
      const migrationFiles = files.filter((file: any) => file.endsWith('.sql')).sort(); // Ensure chronological order
  ...
  async getStatus(): Promise<{
    ...
    const currentVersion = applied.length > 0 ? Math.max(...applied.map((m: any) => m.version)) : 0;
