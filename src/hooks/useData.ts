...
export const campaignToUICampaign = (campaign: Campaign): UICampaign => {
  return {
    ...
    endDate:
      campaign.schedule?.endDate || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
        ...
        if (id && supabase) {
          ...
          if (entityType === 'clients') {
            query = supabase
              .from(entityType as any)
              .select(
                'id, name, description, industry, logo_url, primary_color, secondary_color, brand_guidelines, created_at, updated_at'
              )
              .eq('id', id)
              .single();
          }
    ...
    queryFn: async () => {
      try {
        ...
        if (!response.ok) {
          ...
          console.warn(
            `Campaigns API returned ${response.status} - returning empty array for testing`
          );
          return [];
        }
...
export const useCreateMatrix = () => {
  ...
  const createMatrix = async (matrix: Partial<Matrix>) => {
    ...
    try {
      ...
      const { data, error } = await supabase.from('matrices').insert([matrix]).select().single();
...
export const useFileUpload = () => {
  ...
  const uploadFile = async (file: File, clientId: string, path?: string) => {
    ...
    try {
      ...
      const { data: _data, error } = await supabase.storage.from('assets').upload(filePath, file);
