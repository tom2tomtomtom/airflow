...
export const VideoGenerationPanel: React.FC<VideoGenerationPanelProps> = ({
  ...
  const createPromptFromCombinations = (combinations: any[]) => {
    ...
    const prompt = `Create a ${style} video that showcases: ${headlines.slice(0, 3).join(', ')}. 
    Key messaging: ${copy.slice(0, 2).join('. ')}. 
    Style should be professional and engaging for ${platform} platform.`;
